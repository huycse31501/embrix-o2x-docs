...
13. **Payment Application** (service-payment)
    ...
    **Auto-Charge Flow (CollectionsService.invoiceDueReminder)**  
    End-to-end batch job that automatically charges upcoming invoices for accounts with auto-pay enabled.
    
    1. **Trigger**
       - A scheduled job (e.g., Quartz/Spring scheduler) runs `CollectionsService.invoiceDueReminder` a few days before each invoice due date (e.g., T‑3 days).
    
    2. **Find Eligible Invoices**
       - Query all invoices with:
         - `status = OPEN`
         - `dueDate` within the reminder window
       - For each invoice, load the owning account and its payment profile.
    
    3. **Check Auto‑Charge Configuration**
       - Only continue if the account:
         - Has at least one valid payment method on file, and
         - Has `autoCharge = true` for that profile.
    
    4. **Attempt Charge via payment-gateway**
       - Build a charge request with:
         - `invoiceId`, `accountId`, `amount`, `currency`, and stored payment token.
       - Call `payment-gateway` (e.g., Braintree) to authorize and capture the payment.
    
    5. **If the charge succeeds**
       - Persist a new **Payment** entity linked to the invoice and account.
       - Create **PaymentAllocation** rows to apply that payment to the open invoice balance.
       - Update:
         - `Invoice.status = PAID`
         - `Account.arBalance` decreased by the payment amount.
       - Optionally send a payment confirmation email to the customer.
    
    6. **If the charge fails**
       - Record the failure (reason code, gateway response).
       - Create a notification for follow‑up.
       - Send an email to the customer explaining the failure and asking them to update their payment method.
       - If the invoice is now past due, create a **CollectionUnit** to hand off to Collections (see next item).

- **Collections Management** (if payment fails)
  - **Trigger**: Invoice is past due and no successful payment has been applied.
  - **Flow** (`CollectionsService.createCollectionUnit`):
    1. **Create CollectionUnit**
       - Example payload:
         - `invoiceId`
         - `dueDate: 2026-03-15`
         ...
         - `amount: $26,705`
         - `status: NEW`
    2. **Assign to Collections Queue**
       - Route the `CollectionUnit` to the appropriate collections queue / team based on amount, age, and region.
    3. **Run Dunning Schedule**
       - Send dunning emails at increasing severity on days 1, 7, 14, and 30 past due.
    4. **At 30 days past due**
       - Suspend active services for the account:
         - GraphQL call: `updateSubscriptionStatus(SUSPENDED)`
    5. **At 60 days past due**
       - Create a **write‑off proposal** for finance.
       - Route the proposal for **management approval** before final write‑off.
...
- [Architecture Guide](part1-business-architecture.html)
- [Technical Deep Dive](part2-technical-deep-dive.html)
- [Services & Development](part3-services-development.html)
- [Multi-Tenant Guide](multi-tenant-architecture.html)
